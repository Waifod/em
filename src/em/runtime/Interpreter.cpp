...
Interpreter::Interpreter(std::shared_ptr<std::ostream> outputStream)
    : mOutputStream(std::move(outputStream)) {
  mVariables = {
      {L"ℝ", std::make_unique<values::sets::VirtualSetValue>(
                 std::make_unique<values::functions::NativeFunction>(
                     L"ℝ",
                     [](const auto&) {
                       // TODO: Only return true if it's a number
                       return std::make_unique<values::LiteralValue<bool>>(
                           true);
                     }))},
      ...
      {L"print",
       std::make_unique<values::functions::NativeFunction>(
           L"print",
           [this](const std::vector<std::shared_ptr<values::Value>>& args) {
...
void Interpreter::addVariable(const Token& token,
                              const VisitorRetValue& value) {
...
ast::NodeVisitor::VisitorRetValue Interpreter::visit(
    ast::exprs::FunctionDeclaration* expr) {
  auto function = std::make_shared<values::functions::ProgramFunction>(
      expr->identifier(), expr->parameters(), expr->expression());
  addVariable(expr->identifier(), function);
  return function;
}

ast::NodeVisitor::VisitorRetValue Interpreter::visit(
    ast::exprs::AssignmentExpression* expr) {
...
Interpreter::VisitorRetValue Interpreter::visit(
    ast::exprs::MaterialSetExpression* expr) {
  auto setValue = std::make_unique<values::sets::MaterialSetValue>();
  std::for_each(
      expr->values().cbegin(), expr->values().cend(),
...
Interpreter::VisitorRetValue Interpreter::visit(
    ast::exprs::LiteralExpression* expr) {
  return expr->value();
}

ast::NodeVisitor::VisitorRetValue Interpreter::visit(
    ast::exprs::GroupExpression* expr) {
  return expr->expression()->accept(*this);
}

ast::NodeVisitor::VisitorRetValue Interpreter::visit(
    ast::exprs::VariableExpression* expr) {
  ...
  throw std::logic_error("Undefined identifier '" +
                         utils::string::wStringToString(text) + "'");
}

ast::NodeVisitor::VisitorRetValue Interpreter::visit(
    ast::exprs::FunctionCall* expr) {
  auto function = expr->expression()->accept(*this);
  if (auto* functionValue =
          dynamic_cast<values::functions::Function*>(function.get())) {
    std::vector<VisitorRetValue> args;
    args.reserve(expr->arguments().size());
    std::transform(expr->arguments().cbegin(), expr->arguments().cend(),
                   std::back_inserter(args), [this](const auto& argExpr) {
                     return argExpr->accept(*this);
                   });
